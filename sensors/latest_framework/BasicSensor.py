...
class BasicSensor(IoTElement):
    ...
    def __attest_validate(self, json_update):
        ...
        json_object["event"] = "validateSensor" # Needs to specify the event type
        json_object["message"] = "Sensor validation request"
        json_object["timestamp"] = self.__get_time_stamp()
        print(json_object)
        self.client.publish(f"management", json.dumps(json_object)) # Publish to manager for validation
